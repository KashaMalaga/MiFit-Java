...
public class j {
    ...
    private static final float[] K = new float[ShareData.SHARE_TYPE_LAB_ROPE_SKIPPING_ACCUMULATE];
    private static final float[] L = new float[ShareData.SHARE_TYPE_LAB_ROPE_SKIPPING_ACCUMULATE];
    ...
    static {
        ...
        while (i < 100) {
            float f3;
            float f4 = ((float) i) / CropOverlayView.a;
            float f5 = 1.0f;
            float f6 = f2;
            while (true) {
                f2 = ((f5 - f6) / 2.0f) + f6;
    ...
    static float a(float f) {
        float f2 = Q * f;
        return (f2 < 1.0f ? f2 - (1.0f - ((float) Math.exp((double) (-f2)))) : ((1.0f - ((float) Math.exp((double) (1.0f - f2)))) * (1.0f - 0.36787945f)) + 0.36787945f) * R;
    }

    private float b(float f) {
        return (386.0878f * this.N) * f;
    }

    private double c(float f) {
        return Math.log((double) ((0.35f * Math.abs(f)) / (this.I * this.O)));
    }
    ...
    public void a(int i, int i2, int i3, int i4, int i5, int i6, int i7, int i8) {
        ...
        double e = e(f);
        this.H = (int) (((double) Math.signum(f)) * e);
        this.l = i5;
        this.m = i6;
        this.n = i7;
        this.o = i8;
        ...
        this.h = ((int) Math.round(((double) f2) * e)) + i2;
        ...
        if (((long) currentAnimationTimeMillis) < this.v) {
            ...
            switch (this.a) {
                case l.a /*1*/:
                    ...
                    int i = (int) (CropOverlayView.a * f2);
                    float f3 = 1.0f;
                    f = 0.0f;
                    if (i < 100) {
                        f3 = ((float) i) / CropOverlayView.a;
                        f = ((float) (i + 1)) / CropOverlayView.a;
                        float f4 = K[i];
                        f = (K[i + 1] - f4) / (f - f3);
                        f3 = ((f2 - f3) * f) + f4;
                    }
                ...
                case a.k /*2*/:
                    ...
                    f = this.D == null ? a(f) : this.D.getInterpolation(f);
